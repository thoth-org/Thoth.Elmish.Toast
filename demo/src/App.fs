...
let private switchRenderer (renderer : Renderer) (_: Browser.Types.MouseEvent) =
    match renderer with
    | Renderer.Default -> Browser.Dom.window.location.search <- "?renderer=default"
    | Renderer.Fulma -> Browser.Dom.window.location.search <- "?renderer=fulma"
